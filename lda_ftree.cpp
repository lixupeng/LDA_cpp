...
#pragma omp parallel for schedule(dynamic) private(psum)
    for (int word = 0; word < corpus.n_words; ++word) {
        int thread = omp_get_thread_num();
        psum.resize(n_topics);
		FTree& tree = word_trees[word];
...
double LDA_FTree::loglikelihood(bool test) {
    ...
    for (int doc = 0; doc < corpus.n_docs; ++doc) {
        ...
        for (int topic = 0; topic < n_topics; ++topic) {
            llh += lgamma(count_doc_topic[doc].count(topic) + alpha);
        }
    }
...
void LDA_FTree::experiment() {
    ...
    init();
    int iter = 0;
    fout << "iter\tsample_time\ttotal_time\ttrain_llh\ttest_llh\t" << std::endl;
    while (true) {
        ++iter;
		time_start = clock();
        iterate(false);
        ...
        finish_test();
        std::cout << iter << "\t" << sample_time << "s\t" << total_time << "s\t" << train_llh << "\t" << test_llh << "\t" << std::endl;
        fout << iter << "\t" << sample_time << "\t" << total_time << "\t" << train_llh << "\t" << test_llh << "\t" << std::endl;
        fout.flush();
    }
}
